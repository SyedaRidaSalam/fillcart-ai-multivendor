...
export async function POST(req) {
  ...
  try {
    // 1. NAME & EMAIL LOGIC (No more null null)
    ...
    const isSubscriptionEvent = eventType.startsWith("subscription.");
    let planToSet = undefined; // Default: Update mein kuch nahi badlega

    if (isSubscriptionEvent) {
      const status = evt.data.status;
      // Sirf valid subscription par plus karein
    ...
    await prisma.user.upsert({
      ...
      update: { 
        name: fullName,
        image: evt.data.image_url || "",
        // Agar subscription event HAI, sirf tabhi plan column ko touch karein
    ...
    // 4. CLERK METADATA SYNC (Only for subscriptions)
    if (isSubscriptionEvent && planToSet) {
      try {
        await clerkClient.users.updateUser(userId, {
          publicMetadata: { plan: planToSet },
        });
    ...
    console.log(`✅ Success: ${userId} processed via ${eventType}`);
